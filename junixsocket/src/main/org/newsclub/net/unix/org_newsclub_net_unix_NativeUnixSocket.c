...
//			}
	 ...
	 * Signature: (Ljava/io/FileDescriptor;Lorg/newsclub/net/unix/UserCredentials;)V
	 */
	JNIEXPORT void JNICALL Java_org_newsclub_net_unix_NativeUnixSocket_getUserCredentials
	(JNIEnv * env, jclass clazz, jobject fd, jobject userCredentials) {
		int handle = org_newsclub_net_unix_NativeUnixSocket_getFD(env, fd);
		struct ucred credentials;
		socklen_t credentialsLen = sizeof(credentials);
		int ret = getsockopt(handle, SOL_SOCKET, SO_PEERCRED, &credentials, &credentialsLen);
		if(ret == -1) {
			org_newsclub_net_unix_NativeUnixSocket_throwException(env, strerror(errno), NULL);
		}
		jclass userCredentialsClass = (*env)->GetObjectClass(env, userCredentials);
		jfieldID pidField = (*env)->GetFieldID(env, userCredentialsClass, "pid", "I");
		(*env)->SetIntField(env, userCredentials, pidField, credentials.pid);
		jfieldID uidField = (*env)->GetFieldID(env, userCredentialsClass, "uid", "I");
		(*env)->SetIntField(env, userCredentials, uidField, credentials.uid);
		jfieldID gidField = (*env)->GetFieldID(env, userCredentialsClass, "gid", "I");
		(*env)->SetIntField(env, userCredentials, gidField, credentials.gid);
	}
	
	/*
	 * Class:     org_newsclub_net_unix_NativeUnixSocket
	 * Method:    setPassUserCredentials
	 * Signature: (Ljava/io/FileDescriptor;Z)V
	 */
	JNIEXPORT void JNICALL Java_org_newsclub_net_unix_NativeUnixSocket_setPassUserCredentials
	(JNIEnv * env, jclass clazz, jobject fd, jboolean passUserCredentials) {
		int handle = org_newsclub_net_unix_NativeUnixSocket_getFD(env, fd);
		int optVal = (passUserCredentials == JNI_TRUE) ? 1 : 0;
		int ret = setsockopt(handle, SOL_SOCKET, SO_PASSCRED, &optVal, sizeof(optVal));
		if(ret == -1) {
			org_newsclub_net_unix_NativeUnixSocket_throwException(env, strerror(errno), NULL);
		}
